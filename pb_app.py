...
while display_data.session:
    ...
    if choice == "1":
        ...
        display_data.saved_data.display_result(display_data.data_pb.sql\
            ("SELECT DISTINCT product.name as produit, product.description, \
                product.score as n_s, product.url, product.store as magasins \
                    FROM product INNER JOIN substitute ON product.id = \
                        substitute.healthy_product_id", "all"))
    elif choice == "Q":
