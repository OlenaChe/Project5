                    ...
                    try:
                        if (len(r.json()["products"][i]["product_name_fr"]) \
                            ...
                            len(r.json()["products"][i]["url"]) \
                                > 2 and 
                            len(r.json()["products"][i]["nutrition_grade_fr"])\
                                >= 1 and 
                            len(r.json()["products"][i]["stores"]) > 2):
                            product["name"] = \
                                r.json()["products"][i]["product_name_fr"]
                            product["description"] = \
                            ...
                            product["score"] = \
                                r.json()["products"][i]["nutrition_grade_fr"] 
                            product["store"] = \
                                r.json()["products"][i]["stores"] 
            ...
            with self.connection.cursor() as cursor:
                ...
                try:
                    cursor.execute(sql_data, (prod['name'], \
                        ...
                        prod['score'], prod['category'], prod['store'])) 
                except:
                    pass
        self.commit_connection()
...
class Display_data():
    ...
    def choose_option(self):
        """Method which asks the user to choose the product 
        ...
        self.result = self.data_pb.find_from_table\
            (self.choicecat, "category_id", "product", NUM_PRODUCTS)
        print("")
